...
export const cubicBezier = (t: number, from: number, c1: number, c2: number, to: number) => {
  'worklet';
  const term = 1 - t;
...
export const cubicBezierYForX = (x: number, a: Vector, b: Vector, c: Vector, d: Vector) => {
  'worklet';
  const pa = -a.x + 3 * b.x - 3 * c.x + d.x;
  const pb = 3 * a.x - 6 * b.x + 3 * c.x;
  const pc = -3 * a.x + 3 * b.x;
  const pd = a.x - x;
  const t = solveCubic(pa, pb, pc, pd).filter(root => root >= 0 && root <= 1)[0];
