...
import { DataPoints, Prices, SIZE } from './Model';
...
const buildGraph = (datapoints: DataPoints, label: string) => {
  ...
  const formattedValues = priceList.map(price => [parseFloat(price[0]), price[1]] as [number, number]);
...
const Graph = () => {
  ...
  const animatedProps = useAnimatedProps(() => ({
    d: mixPath(transition.value, previous.path, current.value.path)
  }));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.selection}>
        <View style={StyleSheet.absoluteFill}>
          <Animated.View style={[styles.backgroundSelection, animatedStyles]} />
        </View>
